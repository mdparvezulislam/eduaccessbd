    ...
    >
      ...
      <div className="sticky top-0 z-50 bg-background/80 backdrop-blur-xl border-b -mx-6 px-6 py-3 mb-6 flex items-center justify-between shadow-sm">
        ...
        <Button onClick={handleSubmit} disabled={saving} size="sm" className="bg-green-600 hover:bg-green-700 text-white min-w-[100px] h-8 text-xs">
          {saving ? <Loader2 className="w-3 h-3 animate-spin mr-2"/> : <Save className="w-3 h-3 mr-2"/>} Save
        </Button>
      </div>
          ...
          <Card className="shadow-sm border-t-4 border-t-primary/20">
            ...
            <CardContent className="p-5 space-y-4">
              <div className="space-y-1.5">
                ...
                <Input name="title" value={formData.title} onChange={handleChange} placeholder="e.g. Masterclass 2024" className="font-medium h-9 text-sm"/>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                   ...
                   <div className="flex gap-2">
                      <Input name="slug" value={formData.slug} onChange={(e) => {setSlugManuallyEdited(true); setFormData({...formData, slug: e.target.value})}} className="text-xs h-9" />
