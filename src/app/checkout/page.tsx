  ...
  if (cart.length === 0) {
    return (
      <div className="flex h-[60vh] items-center justify-center flex-col bg-black text-white gap-2">
         ...
         <Button onClick={() => router.push("/")} className="bg-white text-black hover:bg-gray-200">Go Shopping</Button>
      </div>
    );
  }

  // ⚡ Active Payment Data (Auto updates on state change)
              ...
              <Card className="border border-white/10 bg-[#0a0a0a] shadow-xl">
                ...
                <CardContent className="p-4">
                      ...
                      <div key={item.cartId} className="flex justify-between text-xs py-1 border-b border-white/5 last:border-0">
                        <div className="flex gap-2 items-start">
                           ...
                           <div className="flex flex-col">
                             ...
                             <span className="text-[9px] text-green-400">{item.validity || "Standard"}</span>
                           </div>
                        </div>
                  ...
                  <div className="space-y-1 pt-2 border-t border-white/10">
                    ...
                    <div className="flex justify-between text-sm text-gray-400"><span>Method</span><span className="text-gray-200 capitalize">{activePay.name}</span></div>
                  ...
                  {/* Action Button */}
                  <Button type="submit" disabled={loading} className="w-full mt-5 h-10 text-sm font-bold bg-white text-black hover:bg-gray-200 transition-all active:scale-[0.98] uppercase tracking-wide">
                    {loading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <>Confirm Order <ArrowRight className="ml-2 h-4 w-4" /></>}
                  </Button>
